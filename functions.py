...
def plot_PCA_jitter(mod, ax=None):
    from copy import deepcopy
    from sklearn.decomposition import PCA
    figsize = 6
    species = mod.comm[0]
    ...
    # Add a small amount of random noise to the x and y coordinates
    jittered_PCs = norm_PCs + np.random.normal(0, 0.01, norm_PCs.shape)

    if ax is None:
        fig, ax = plt.subplots(figsize=(figsize, figsize), dpi= 80, facecolor='w', edgecolor='k')
    ax.scatter(jittered_PCs[:,0], jittered_PCs[:, 1], color = PC_colors/255.0)  # use PC_colors as colors and set alpha to 0.5
    ax.set_xlabel('genetic PC 1')
    ax.set_ylabel('genetic PC 2')
...
def draw_landscape(d=10):
    ...
    # Create a matrix to store the drawing
    ...
    # Create the 2D array
    landscape_2d = np.array([[color_to_value.get(tuple(drawing_matrix[i, j]), 0) for j in range(drawing_matrix.shape[1])] for i in range(drawing_matrix.shape[0])])
